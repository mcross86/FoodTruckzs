...
/** Human-readable RFQ number for display (no prefix). */
export function formatRfqNumber(rfqNumber: number): string {
  return String(rfqNumber);
}

/** Preferred path/query identifier: public RFQ number. */
export function rfqPublicIdentifier(rfq: { rfqNumber: number }): string {
  return formatRfqNumber(rfq.rfqNumber);
}
